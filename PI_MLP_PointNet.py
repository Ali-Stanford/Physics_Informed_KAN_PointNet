...
Epoch = 3000
Nb = 10 #batch size (memory sensitive)
J_Loss = 0.035
...
def TheLoss(model,X,pose_BC,pose_sparse,pose_interior,pose_BC_temperature_p,pose_BC_p,pose_sparse_p,pose_interior_p):
    ...
    u = Y[:, 0, :].contiguous().view(-1)
    v = Y[:, 1, :].contiguous().view(-1)
    p = Y[:, 2, :].contiguous().view(-1)
    T = Y[:, 3, :].contiguous().view(-1)

    u_in = u[pose_interior_p]
    v_in = v[pose_interior_p]
    T_in = T[pose_interior_p]
    ...
    dv_dx = dv_dx.contiguous().view(-1)
    dv_dy = dv_dy.contiguous().view(-1)
    dp_dx = dp_dx.contiguous().view(-1)
    dp_dy = dp_dy.contiguous().view(-1)
    dT_dx = dT_dx.contiguous().view(-1)
    dT_dy = dT_dy.contiguous().view(-1)

    du_dx_in = du_dx[pose_interior_p]
    du_dy_in = du_dy[pose_interior_p]
    dv_dx_in = dv_dx[pose_interior_p]
    dv_dy_in = dv_dy[pose_interior_p]
    dp_dx_in = dp_dx[pose_interior_p]
    ...
    d2v_dx2 = d2v_dx2.contiguous().view(-1)
    d2v_dy2 = d2v_dy2.contiguous().view(-1)
    d2T_dx2 = d2T_dx2.contiguous().view(-1)
    d2T_dy2 = d2T_dy2.contiguous().view(-1)

    d2u_dx2_in = d2u_dx2[pose_interior_p]
    d2u_dy2_in = d2u_dy2[pose_interior_p]
    d2v_dx2_in = d2v_dx2[pose_interior_p]
    d2v_dy2_in = d2v_dy2[pose_interior_p]
...
def build_model_Thermal():
    ...
    for epoch in range(Epoch):
        ...
        for sb in range(int(np.ceil(data / Nb))):
            ...
            # Forward passs
            cost_value = TheLoss(model, X_train_mini, group_BC,group_sparse,group_interior,group_BC_temperature_p,group_BC_p,group_sparse_p,group_interior_p)
            cost_value.backward()
            optimizer.step()
